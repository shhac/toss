...
fn parseArgs(allocator: std.mem.Allocator, args: []const [:0]const u8) ArgParseError!Config {
    var config = Config{};
...
fn run(init: std.process.Init) !void {
    ...
    for (config.dice_specs) |spec_str| {
        const expr = parser.parse(spec_str) catch |parse_err| {
            stderr_term.setColor(.red) catch {};
            ...
            switch (parse_err) {
                ...
                error.DivisionByZero => try err_out.print("Division by zero in '{s}'\n", .{spec_str}),
        ...
        formatExpr(&label_stream, expr) catch {};
        const label_len = label_stream.end;
    ...
    for (parsed_exprs.items) |expr| {
        ...
        if (config.result_only) {
            stdout_term.setColor(.bold) catch {};
            try out.print("{d}", .{result.total});
            stdout_term.setColor(.reset) catch {};
            try out.print("\n", .{});
            row_index += 1;
            continue;
        }
        ...
        if (!config.no_labels) {
            stdout_term.setColor(group.label) catch {};
            try out.print("[", .{});

            // Format the expression to get its length, then pad with underscores
            var expr_label_stream: std.Io.Writer = .fixed(&label_buf);
            try formatExpr(&expr_label_stream, expr);
            const expr_len = expr_label_stream.end;
            ...
            try out.print("{s}", .{label_buf[0..expr_len]});
            try out.print("]", .{});
            stdout_term.setColor(.reset) catch {};
        }
        ...
        for (result.dice_rolls) |dice_result| {
            for (dice_result.dice_results) |die| {
                ...
                if (die.kept) {
                    ...
                    stdout_term.setColor(.reset) catch {};
                    stdout_term.setColor(result_color) catch {};
                    ...
                    if (dice_result.sides == 0) {
                        ...
                        if (config.no_labels) {
                            ...
                            try formatFudgeValue(out, die.value, 0);
                        } else {
                            try formatFudgeValue(out, die.value, sides_width);
                        }
                    } else if (config.no_labels) {
                        // No padding when labels are omitted
                        try out.print("{d}", .{die.value});
                    } else {
                        try writeRightAligned(out, die.value, sides_width, ' ');
                    }
                    ...
                    stdout_term.setColor(.dim) catch {};
                    if (dice_result.sides == 0) {
